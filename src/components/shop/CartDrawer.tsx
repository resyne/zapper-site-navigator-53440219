            ...
            <>
                  ...
                  {items.map((item) => (
                    <div key={item.variantId} className="flex gap-4 p-2 border-b border-border">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="font-semibold text-foreground">€{parseFloat(item.price.amount).toFixed(2)}</p>
                      </div>
              ...
              <div className="flex-shrink-0 space-y-4 pt-4 border-t bg-background">
                ...
                <Button onClick={handleCheckout} className="w-full" size="lg" disabled={items.length === 0 || isLoading || isSyncing}>
                  {isLoading || isSyncing ? <Loader2 className="w-4 h-4 animate-spin" /> : <><ExternalLink className="w-4 h-4 mr-2" />Checkout con Shopify</>}
                </Button>
              </div>
            </>
          )}
