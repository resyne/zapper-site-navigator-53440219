        ...
        <div className="grid gap-8 lg:grid-cols-2 lg:gap-16 items-center">
          ...
          <div className="grid gap-3 sm:gap-4 sm:grid-cols-2">
            {problems.map((problem, index) => (
              <div
                key={problem.title}
                className={`group p-4 sm:p-5 bg-white/10 backdrop-blur-sm rounded-lg sm:rounded-xl border border-white/20 hover:border-primary/30 hover:bg-white/15 transition-all duration-300 animate-fade-in-up animation-delay-${(index + 1) * 100}`}
              >
